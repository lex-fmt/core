...
use crate::lex::ast::{Container, ContentItem, Document};
...
pub(crate) fn build_document_content_parser(
    ...
    recursive(move |items| {
        ...
        let single_item = {
            ...
            choice((
                foreign_block(source.clone()).map(|fb| ContentItem::ForeignBlock(Box::new(fb))),
                annotation_parser,
                list_parser,
                definition_parser,
                session_parser,
...
mod tests {
    use super::*;
    use crate::lex::ast::{AstNode, Position};
    ...
    fn test_real_content_extraction() {
        ...
        let doc = crate::lex::parsers::parse_document(input).expect("Failed to parse");
