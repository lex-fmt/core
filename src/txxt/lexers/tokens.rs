...
pub enum Token {
    ...
    IndentLevel(Vec<(Token, std::ops::Range<usize>)>),
    DedentLevel(Vec<(Token, std::ops::Range<usize>)>),
...
impl fmt::Display for Token {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let name = match self {
            ...
            Token::Indent => "indent",
            Token::IndentLevel(_) => "indent-level",
            Token::DedentLevel(_) => "dedent-level",
...
mod tests {
    use super::*;
    use crate::txxt::lexers::tokenize;
    ...
    fn test_token_predicates() {
        ...
        assert!(Token::IndentLevel(vec![]).is_indent_level());
        assert!(Token::DedentLevel(vec![]).is_dedent_level());
        assert!(!Token::Text("".to_string()).is_indent());

        assert!(Token::Indent.is_whitespace());
        assert!(Token::IndentLevel(vec![]).is_whitespace());
        assert!(Token::DedentLevel(vec![]).is_whitespace());
