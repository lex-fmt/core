...
## Project Overview and Docs
    ...
    docs/dev/guides/on-all-of-lex.lex
...
Follow `rustfmt` defaults (4-space indentation, trailing commas for multi-line literals). Prefer snake_case files and modules, CamelCase types, and SCREAMING_SNAKE_CASE constants to match existing modules. Keep parser stages pure and retry-friendly; pass shared state via structs instead of globals. Document grammar nuances in `///` doc comments and mirror the tone from `docs/specs` (terse, factual). New command-line flags should be added through Clap derives in `src/bin/lex.rs` and reflected in the specs. Use scripts/pre-commit
