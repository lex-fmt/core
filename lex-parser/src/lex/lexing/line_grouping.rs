...
use crate::lex::lexing::line_classification::{classify_line_tokens, VerbatimClassifier};
...
pub fn group_into_lines(tokens: Vec<(Token, ByteRange<usize>)>) -> Vec<LineToken> {
    ...
    let mut verbatim_classifier = VerbatimClassifier::new();
    for (token, span) in tokens {
        ...
        if matches!(token, Token::Indent(_) | Token::Dedent(_)) {
            ...
            if !current_line.is_empty() {
                let line = classify_and_create_line_token(current_line);
                line_tokens.extend(verbatim_classifier.process_line(line));
            }
    ...
    // Flush the verbatim classifier to get any remaining lines.
    line_tokens.extend(verbatim_classifier.flush());
...
mod tests {
    ...
    fn test_group_with_indent_dedent() {
        let tokens = vec![
            ...
            (Token::BlankLine(Some("\n".to_string())), 6..7),
            (
                Token::Indent(vec![(Token::Indentation, 7..11)]),
                0..0,
            ),
