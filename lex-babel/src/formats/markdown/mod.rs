...
//!     This means that the parsing code must assemble a session's content from the flat markdown tokens
//!     infering the 2d structure from the 1d nodes is not possible, so we will use heuristics to guess,
//!     but keeping a stack with session levels and infering a tree from that.
//!
//!
...
//!     Note that the lex-parser crate has a robust testing toolset, including ast assertions that can verify a number of ast nodes and their data in a fluent way, use it. This also shields us from ast changes breaking every test.
