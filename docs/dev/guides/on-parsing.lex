Parsing Lex Strings
	...
	The parser receives a flat Vec<(Token, Range<usize>)> from the lexing pipeline. The linebased implementation builds its own tree structure internally before returning the AST.
...
Parser Integration
	...
	The linebased parser is fully integrated with the common AST construction code:
