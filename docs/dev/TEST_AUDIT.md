...
|----------------|-------|------------|-------------|
| `hardcoded_source` | 22 | 5.3% | Tests using ad hoc hardcoded lex source strings |
...
1. `src/lex/ast/location.rs` - 12 tests with `no_source` violations
2. `src/lex/lexers/linebased/pipeline.rs` - 17 tests with `hardcoded_source` violations
...
- **Focus:** `no_source` violations in `src/lex/ast/location.rs`
...
- **Strategy:** Create reusable test fixtures in `src/lex/testing/` module
...
- `src/lex/ast/location.rs` - 12 tests

**Batch 3a:** Lexer pipeline tests

- `src/lex/lexers/linebased/pipeline.rs` - 17 tests
