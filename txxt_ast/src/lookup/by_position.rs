...
use crate::error::PositionLookupError;
use crate::node::{AstNode, ContentItem, Document};
...
pub fn format_at_position(
    ...
    let (line, column) = parse_position(position_str).map_err(|e| {
        PositionLookupError::InvalidPositionFormat(format!(
...
fn find_and_format_at_position(
    ...
    if elements.is_empty() {
        return Err(PositionLookupError::NotFound(format!(
...
mod tests {
    ...
    fn test_format_at_position_missing_position() {
        ...
        if let Err(PositionLookupError::InvalidPositionFormat(msg)) = result {
    ...
    fn test_find_position_with_span() {
        ...
        let doc = Document::with_content(vec![ContentItem::Paragraph(para)]);
