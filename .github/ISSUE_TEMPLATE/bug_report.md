# Bug Report: Verbatim Groups Not Parsed Correctly with Blank Lines Between Pairs
...
**Why it works**: The pattern `subject → blank line → content → subject → blank line → content` is correctly recognized by the parser.
...
**The Problem**: When there are blank lines between pairs, the repetition logic may not correctly recognize subsequent subjects as part of the same group. Possible issues:

1. **Content boundary detection**: The content parser (`with_content`) expects `Indent → content → Dedent`. When content ends with a `Dedent`, followed by blank lines, then another subject at the same level, the parser may not recognize this as continuing the verbatim group.

2. **Repetition logic**: The `subject_content_pair.repeated()` may stop after the first pair when it encounters blank lines, thinking the verbatim block has ended.

3. **Token consumption**: Blank lines between pairs may be consumed in a way that prevents the repetition from continuing to match the next subject.
...
- Tests: `tests/elements_verbatim.rs` (lines 272-310)
...
2. **Content boundary detection**: The content parser should correctly stop when it encounters another subject at the same indentation level as the first subject, even if there are blank lines in between
3. **Dedent handling**: When content ends and we see a subject at the same indentation level as the first subject, recognize this as the start of another pair in the group
...
- ❌ `verbatim-13-group-spades.lex` - Fails (blank lines between pairs, no blank line after first subject)
- ❌ `verbatim-12-document-simple.lex` - Fails (verbatim groups within sessions)
