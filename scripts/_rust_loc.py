...
if len(sys.argv) < 2:
    print("Usage: _rust_loc.py <tmpfile>", file=sys.stderr)
    sys.exit(1)
...
for f in files:
    ...
    if len(name) > 60:
        name = name[:57] + '...'
    print(f"| {name} | {f['main_code']:>4} | {f['test_code']:>4} | {total:>4} |")

# Calculate totals
total_code = sum(f['main_code'] for f in files)
total_tests = sum(f['test_code'] for f in files)
total_all = total_code + total_tests
print(f"| TOTAL ({len(files)} files) | {total_code:>4} | {total_tests:>4} | {total_all:>4} |")
print()
print()
...
for d in sorted_dirs:
    ...
    total = code + tests
    print(f"| {d}/ | {code:>4} | {tests:>4} | {total:>4} |")

print(f"| TOTAL (aggregated by directory) | {total_code:>4} | {total_tests:>4} | {total_all:>4} |")
